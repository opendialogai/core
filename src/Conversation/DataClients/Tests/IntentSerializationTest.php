...
class IntentSerializationTest extends SerializationTestCase
{
    public function testNormalizeStandaloneIntent()
    {
        ...
        $expected = [
            ...
            'behaviors' => [], 'created_at' => $intent->getCreatedAt()->format(\DateTime::ISO8601),
            'updated_at' => $intent->getUpdatedAt()->format(\DateTime::ISO8601), 'speaker' => $intent->getSpeaker(),
            'confidence' => $intent->getConfidence(), 'sample_utterance' => $intent->getSampleUtterance(), 'transition' => [
                'conversation' => $intent->getTransition()->getConversation(), 'scene' => $intent->getTransition()->getScene(),
                'turn' => $intent->getTransition()->getTurn()
            ], 'expected_attributes' => $intent->getExpectedAttributes(), 'actions' => [],
            'listens_for' => $intent->getListensFor(), 'virtual_intents' => $intent->getVirtualIntents()->map(fn($i) => [
    ...
    {
        $normalizers = [
            new IntentNormalizer(), new ConditionCollectionNormalizer(), new ConditionNormalizer(), new
    ...
    public function testDenormalizeMissingType()
    {
        ...
        $intent = $this->getStandaloneIntent();
        $data = [
            'uid' => $intent->getUid(), 'od_id' => $intent->getOdId(), 'name' => $intent->getName(),
            'description' => $intent->getDescription(), 'interpreter' => $intent->getInterpreter(), 'conditions' => [],
            'behaviors' => [], 'created_at' => $intent->getCreatedAt()->format(\DateTime::ISO8601),
            'updated_at' => $intent->getUpdatedAt()->format(\DateTime::ISO8601), 'speaker' => $intent->getSpeaker(),
            'confidence' => $intent->getConfidence(), 'sample_utterance' => $intent->getSampleUtterance(), 'transition' => [
